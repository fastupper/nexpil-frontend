...
export class Calendar extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      myappdate: null,
      selectAllTimeSlotStatus: true,
      time_slot_List :[],
      selected_slot_Item_List : [],
      // today: date.now()
    };
  ...
  async fetchAllTimeSlotList() {
    ...
    this.setState({time_slot_List: a });
  }

  async fetchMyAvaliableTimeSlot(e) {
    const {myappdate,time_slot_List} = this.state;
    ...
    this.setSelectedTimeSlotStatus(a);
  }
  setSelectedTimeSlotStatus = (mylist)=>{
    const{time_slot_List} = this.state;
    for(let i = 0; i < time_slot_List.length; i ++){
      let check_status = mylist.filter((item)=>item['time_slot_id'] === time_slot_List[i]['time_slot_id']);
      if(check_status.length > 0){
        this.setState({["select_time_slot_item_"+time_slot_List[i]['time_slot_id']]:false});
      }
      else {
        this.setState({["select_time_slot_item_"+time_slot_List[i]['time_slot_id']]:true});
      }
    }
  }
  ...
  async fetchTimeSlot() {
    ...
    let appointmentDetail = {
      appointment_date: this.state.myappDate !== undefined && this.state.myappDate !== null ? this.state.myappDate.toISOString().substring(0, 10):mid_date.toISOString().substring(0, 10),
    }
    var temptime = await axios.post(server.serverURL + "v1/available-time-slots", appointmentDetail, {
      headers: {
        'Content-Type': 'application/json',
  ...
  initfunc=async()=>{
    await this.fetchAllTimeSlotList();
  ...
  handleDateChangeformyappdate =async (e)=>{
    ...
    await this.fetchMyAvaliableTimeSlot(e);
  }
  handleModalHide() {
    this.setState({ modalShow: false })
  }
  ...
  selectAllTime=(str)=>{
    ...
    if(str ==="deselect"){
      for(let i = 0; i < time_slot_List.length; i ++){
        let mid_id = time_slot_List[i]['time_slot_id'];
        this.setState({["select_time_slot_item_"+mid_id]:false});
      }
      this.setState({selected_slot_Item_List:[],selectAllTimeSlotStatus: false});
    }
    else{
      for(let i = 0; i < time_slot_List.length; i ++){
        let mid_id = time_slot_List[i]['time_slot_id'];
        this.setState({["select_time_slot_item_"+mid_id]:true});
      }
      this.setState({selectAllTimeSlotStatus: true});
    }
  }
  timeSlotItemSelected=async(item)=>{
    let mid_id = item !== undefined && item['time_slot_id'] !== undefined ? item['time_slot_id']:null;
    if(mid_id !==null){
      let mystatus = this.state["select_time_slot_item_"+mid_id];
      this.setState({["select_time_slot_item_"+mid_id]:!mystatus});
      if(mystatus){
        // await this.addtimeslotitem(item);
      }
      else {
        // await this.removeMyTime(item);
      }
      
    }
  }
  addtimeslotitem=async(item)=>{
    ...
    await axios.post(server.serverURL + 'v1/remove-available-time-slots', appointmentDetail, config)
      .then(res => {
        this.fetchMyAvaliableTimeSlot();
      })
      .catch(error => {
  ...
  SaveSelectedTimeSlot=async()=>{
    ...
    for(let i = 0; i < time_slot_List.length; i ++){
      ...
      if(mystatus === true){
        res.push(mid_id);
        // await this.removeMyTime(time_slot_List[i]);
      }
      else {
        // await this.addtimeslotitem(time_slot_List[i]);
      }
    }
    await this.addTimeList(res);
  }
  addTimeList=async(list)=>{
    var tzoffset = this.state.myappdate.getTimezoneOffset() * 60000; //offset in milliseconds
    var localISOTime = (new Date(this.state.myappdate - tzoffset)).toISOString().substring(0, 10);
    ...
    await axios.post(server.serverURL + 'v1/add-available-time-slots-list', appointmentDetail, config)
      .then(res => {
  ...
  render() {
    const {selectAllTimeSlotStatus,time_slot_List,myavaliabletimeslot} = this.state;
      ...
      <div className='demo-app'>
        ...
        >
          ...
          <Modal.Footer>
            ...
            {this.state.showavaliableModal ? (
              <div className="custom-modal-avaliable-timeslot-container">
                <div className="custom-modal-avaliable-timeslot-body">
                  <div className="custom-modal-avaliable-timeslot-item-body">
                    <div className='custom-modal-avaliable-timeslot-title'>
                      <div className="custom-modal-avaliable-timeslot-title-body">My Avaliablity</div>
                      <span aria-hidden="true" style={{fontSize: 28,fontWeight: 800,paddingTop: 10,cursor:'pointer'}} onClick={this.closeAvaliableModal}>×</span>
                    </div>
                    <div className="custom-modal-avaliable-timeslot-main-body">
                      <div className="custom-modal-avaliable-timeslot-item-one">
                        <div style={{ background: 'rgb(224 225 225 / 68%)',padding: 5,borderRadius: 10}}>
                          <div style={{fontSize: 10,fontWeight: 800,fontFamily: 'boldTitles',color: 'gray'
                          }}>Date</div>
                          <div className="form-group">
                            <DatePicker
                              ref={this.appointmentDate}
                              ...
                              name="startDate"
                              id = "avaliabletimeslotdate"
                              dateFormat="MMM,dd yyyy"
                            />
                          </div>
                        </div>
                        <div style={{ background: 'rgb(224 225 225 / 68%)',padding: 10,borderRadius: 10,marginTop: 10}}>
                          <div style={{fontSize: 12,fontWeight: 800,fontFamily: 'regularText',color: 'black',paddingTop: 5}}>Choose your date</div>
                          <div style={{fontSize: 12,fontWeight: 800,fontFamily: 'regularText',color: 'black',paddingTop: 15}}>Select the times you are available</div>
                          <div style={{fontSize: 12,fontWeight: 800,fontFamily: 'regularText',color: 'black',paddingTop: 15}}>Click Save</div>
                        </div>

                        <div style={{ background: 'rgb(224 225 225 / 68%)',padding: 10,borderRadius: 10,marginTop: 10}}>
                          <div style={{fontSize: 12,fontWeight: 800,fontFamily: !selectAllTimeSlotStatus ? 'boldTitles':'regularText',color: 'black',paddingTop: 5,cursor:"pointer" ,fontWeight : !selectAllTimeSlotStatus ? "800":"100"}} onClick ={()=>this.selectAllTime('deselect')}>Deselect All</div>
                          <div style={{fontSize: 12,fontWeight: 800,fontFamily: selectAllTimeSlotStatus ? 'boldTitles':'regularText',color: 'black',paddingTop: 15,cursor:"pointer",fontWeight : selectAllTimeSlotStatus ? "800":"100"}} onClick ={()=>this.selectAllTime('select')}>Select All Times</div>
                        </div>
                      </div>
                      <div className="custom-modal-avaliable-timeslot-item-second">
                          {time_slot_List ? time_slot_List.map((item,index)=>{
                            let check_item = item['time_slot'] !== undefined && item['time_slot'] !== null && item['time_slot'].split(":");
                            let check_item_status = false;
                            
                            let am_pm = parseInt(check_item[0]) >=12 ? "PM":"AM";
                            let myTime = check_item[0]+":"+check_item[1] +" " + am_pm;
                            if(check_item[1]==="00"){
                              check_item_status = true;
                            }

                            let selected_item_status = this.state['select_time_slot_item_'+item['time_slot_id']];
                            return (
                              <div key = {index} 
                                style={{ width:check_item_status ? "100%" :"90%",background: !selected_item_status ? 'rgb(224 225 225 / 68%)':'rgb(124 14 245)',padding: 10,borderRadius: 10,fontFamily: 'revert',marginTop: 2,marginLeft:check_item_status ? '0':'10%',
                                cursor:'pointer',color:selected_item_status ? "white":"black"
                              }}
                                onClick={()=>this.timeSlotItemSelected(item)}
                              >
                                <div style={{fontFamily: selected_item_status ? "fantasy":'revert'}}>
                                  {myTime}
                                </div>
                              </div>
                            )
                    ...
                    <Button variant="primary" style={{ background: "rgb(124 14 245)",fontFamily: 'revert', borderColor: "transparent", borderRadius: "20px",width:"100%" }} onClick={this.SaveSelectedTimeSlot}>
