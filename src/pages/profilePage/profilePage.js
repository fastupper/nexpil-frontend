...
export const ProfilePage = () => {
  ...
  const getAvatar = () => {
    axios
      .get(server.serverURL + "v1/profile", {
        headers: {
  ...
  const onFileUpload = () => {
    if (avatarStatus == true) {
      ...
      axios
        .post(server.serverURL + "v1/change-avatar", formData, {
          headers: {
  ...
  const changePassword = () => {
    axios
      .post(server.serverURL + "v1/change-password", formData, {
        headers: {
          Authorization: "Bearer " + token,
    ...
    axios
      ...
      .then((res) => {
        localStorage.setItem(
          "profileDescription",
          res.data.data.profile_description
        );
        setAvatar(res.data.data.avatar);
        ...
        <div className="profile-form">
          ...
          >
            <div style={{ textAlign: "center" }}>
              <img
                className="profile-avatar img-circle avatar-size"
                src={avatar}
              />
              <div className="d-flex">
                ...
                {!avatarStatus && (
                  <input
                    className="btn btn-default w-75 m-0"
                    type="file"
                    onChange={onFileChange}
                    style={{ display: avatarStatus }}
                  />
                )}
              </div>
              <p className="profile-name pt-1">
            ...
            <div>
              {avatarStatus && (
                <textarea
                  disabled
                  className="profile-textfield form-control"
                  style={{ borderRadius: 15, borderColor: "transparent" }}
                  rows="5"
                  cols="40"
                >
                  {profileDescription}
              ...
              {!avatarStatus && (
                <textarea
                  ...
                  rows="5"
                  cols="40"
                  placeholder="Profile - 2/200 words"
                >
                  {profileDescription}
                </textarea>
              )}
              <div
                ...
                onClick={!avatarStatus && saveProfile}
                style={{
                  float: "right",
                  marginTop: 12,
                  marginBottom: 12,
